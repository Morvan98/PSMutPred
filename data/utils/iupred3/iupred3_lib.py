...
fmf 5.23 modified
'''
matrix_preloaded = read_matrix("{}/data/iupred2_short_energy_matrix".format(PATH))
histo_preloaded, histo_min_preloaded, histo_max_preloaded, histo_step_preloaded = read_histo("{}/data/long_histogram".format(PATH))
#####
...
def iupred(seq, mode='long', smoothing='medium'):
    if mode == "short":
        ...
        uc = 100
        wc = 15
        # mtx = read_matrix("{}/data/iupred2_long_energy_matrix".format(PATH)) ### modified by fmf
        # histo, histo_min, histo_max, histo_step = read_histo("{}/data/long_histogram".format(PATH)) ### modified by fmf
        mtx = matrix_preloaded
        histo, histo_min, histo_max, histo_step = histo_preloaded, histo_min_preloaded, histo_max_preloaded, histo_step_preloaded
    ...
    if mode == 'short':
        ...
        for idx in range(len(seq)):
            start = max(0, idx - wc)
            end = min(len(seq), idx + wc + 1)
            weighted_energy_score[idx] = avg(unweighted_energy_score[start:end])
